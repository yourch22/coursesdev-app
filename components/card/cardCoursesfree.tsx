...
export const CardCoursesfree = ({coursesfree}) => {
    moment.locale('es');
    return (
        <div className="mt-1">
        
        <Link key={coursesfree.id} href="udemy-free/[...params]" as={`udemy-free/${coursesfree.id}-${coursesfree.published_title}`} className="c-card block bg-white shadow-md hover:shadow-xl rounded-lg overflow-hidden">
        
            <div className="relative pb-48 overflow-hidden">
                <img loading="lazy" className="absolute inset-0 h-full w-full object-cover"
                    src={coursesfree.image_240x135} alt=""/>
            ...
            <div className="p-4">
                <h2 className="mt-2 mb-2  font-bold">{coursesfree.title}</h2>
                <p className="text-sm">Creado por: <i>{coursesfree.visible_instructors[0].display_name}</i></p>
                <div className="mt-2 flex items-center text-sm text-amber-500">
                    <span className="inline-block text-white bg-amber-500 font-bold text-sm py-0 px-1 rounded">{coursesfree.avg_rating.toPrecision(2)}</span>
         
                    <div className="relative inline-flex text-amber-500">
                        <div className="absolute top-0 left-0 whitespace-no-wrap overflow-hidden w-0" style={{ width: (coursesfree.avg_rating*100)/5 }}>
