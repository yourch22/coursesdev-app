...
export const getServerSideProps = async ({ query }) => {
    ...
    const rutacategory = query.category;
    let page = query.page ? query.page: 1;
    const page_size = 20;
    const type = "category";
    const price = "price-paid";
    const language = "es";
    ...
    const category= categoryByRoute[0]['api'] || 'Development';
    ...
    return {
        props: {
            searchCategory,rutacategory,page,page_size,categoryByRoute
        }
    }
}
type Props = {
    searchCategory,search,page,page_size,categoryByRoute
}
const PageCategory = ({searchCategory,rutacategory,page,page_size,categoryByRoute}) => {
    const router = useRouter();
    const [valuesearch, setValuesearch] = useState("");
    const totalCourses= searchCategory.count;
    const routePaginate= `/courses/${rutacategory}${'?page=' ? '?page=': null}`;
