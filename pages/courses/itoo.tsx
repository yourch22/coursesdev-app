...
import { CardItoo } from '../../components/card/cardItoo';
...
export const getServerSideProps = async ({ query }) => {
    ...
    const { data: itoo } = await axios.get(
        `${url}/itoo/?page=${page}`,{
            headers:{
        ...
        <Layout>
            <>
                ...
                <div className="w-screen sm:w-full md:full lg:w-full xl:w-full mx-auto grid grid-cols-1 px-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    {item.map(itoo => (
                        <CardItoo key={itoo.id} data={itoo} />
                    ))}
                </div>
                <div className="flex justify-center space-x-3 text-center my-6">
                    {/* <button className="text-base sm:text-xs md:text-sm lg:text-lg xl:text-xl font-semibold bg-indigo-900 flex items-center mx-8 px-6 text-white rounded-lg py-2 block shadow-xl hover:bg-indigo-500">Carga más */}
                    {/* </button>  */}

                    <div className="inline-flex border center border-[#e4e4e4] bg-white p-4 rounded-xl">
                        <ul className="flex items-center -mx-[6px]">
                            <li className="px-[6px]">
                                <a href={`/courses/itoo?page=${parseInt(page) - 1 || 1}`} className="w-auto h-9 flex items-center justify-center rounded-md border border-[#EDEFF1] text-[#838995] text-base hover:bg-primary hover:border-primary hover:text-white">
                                    <span>
                                    ...
                                    <span className="ml-2 mr-2">Anterior</span>
                                </a>
                            </li>
                            <li className="px-[6px]">
                                <a href="#" className="mr-2 ml-2 w-9 h-9 flex items-center justify-center rounded-md border border-[#EDEFF1] text-[#838995] text-base hover:bg-primary hover:border-primary hover:text-white">
                            ...
                            <li className="px-[6px]">
                                <a href={`/courses/itoo?page=${parseInt(page) + 1 || 1}`} className="w-auto h-9 flex items-center justify-center rounded-md border border-[#EDEFF1] text-[#838995] text-base hover:bg-primary hover:border-primary hover:text-white">
                                    <span className="mr-2 ml-2">Siguiente</span>
                                    <span>
                                        <svg width="8" height="15" viewBox="0 0 8 15" className="fill-current stroke-current">
