...
import mainLogoUdemy from '../../public/assets/logoudemy.png';
...
export const getServerSideProps = async ({query})=>{
    ...
    const courseUrl = `${process.env.API_URL}/coursescupon/`;
    const coursesOutstandingUrl = `${process.env.API_URL}/coursescupon/coursesoutstanding`;
    const relatedCourseUrl = `${process.env.API_URL}/coursescupon/related/`;
    const [resCourses,resOutstanding,resRelated] = await Promise.all([fetch(courseUrl + query.ruta, {
        headers: {'Authorization': `Basic ${token}`}
      }),fetch(coursesOutstandingUrl,{
        headers: {'Authorization': `Basic ${token}`}}),fetch(relatedCourseUrl+query.ruta,{
            headers: {'Authorization': `Basic ${token}`}})]);
    const [course,outstanding,relatedCourses] = await Promise.all([resCourses.json(),resOutstanding.json(),resRelated.json()]);
    return {
        props: {
            course,outstanding,relatedCourses
        }
    }
}

const CouponsDetail = ({course,outstanding,relatedCourses}) => {
    return (
        <Layout>
            {course.map((item)=>(
            <div key={item.id} className="flex justify-between container mx-auto">
            <div className="w-full lg:w-8/12">
                ...
                <div className="pb-12 md:pb-12">
                    <article className="max-w-sm mx-auto md:max-w-none grid md:grid-cols-2 gap-6 md:gap-8 lg:gap-12 xl:gap-16 items-center">
                        <Link href="blog-post" className="relative block group" data-aos="fade-right" data-aos-delay="200">
                            ...
                            <img className="rounded w-full h-full object-cover transform hover:scale-105 transition duration-700 ease-out" src={item.portada} alt="Blog post 01" />
                        ...
                        <div data-aos="fade-left" data-aos-delay="200">
                            <header>
                                ...
                                <h3 className="h3 text-2xl lg:text-3xl mb-2">
                                    <Link href="/blog-post" className="hover:text-gray-500 transition duration-150 ease-in-out">{item.titulo}</Link>
                                </h3>
                            </header>
                            <p className="text-lg text-gray-500 grow">{item.descripcion_corta}</p>
                            <div className="mt-2 flex flex-wrap items-center justify-between text-sm text-amber-500">
                                <div className="flex items-center space-x-2">
                                    <span className="inline-block text-white bg-amber-500 font-bold text-sm py-0 px-1 rounded">{item.calificacion.toPrecision(2)}</span>
                                    <div className="relative inline-flex text-amber-500">
                                        <div className="absolute top-0 left-0 whitespace-no-wrap overflow-hidden w-0" style={{ width: (item.calificacion*100)/5}}>
                                    ...
                                    <span className="inline-block text-gray-500 font-bold text-sm py-0 px-1">{item.cant_estudiantes}</span>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <div className="inline-flex text-sm font-medium bg-rose-100 text-rose-600 rounded-full text-center px-2 py-0.5">${item.precio_oferta}</div>
                                    <div className="inline-flex text-sm font-medium text-slate-500 line-through">${item.precio}</div>
                                </div>
                            </div>
                            <footer className="mt-2 flex flex-wrap items-center justify-between">
                                <div className="flex items-center space-x-2">
                                <Link href="#">
                                    <Image className="rounded-full shrink-0 mr-4" src={mainLogoUdemy} width="40" height="40" alt="Author 04" />
                                </Link>
                                    <Link href="#" className="font-medium text-gray-500 hover:text-gray-400 transition duration-150 ease-in-out">{item.instructor}</Link>
                                </div>
                                <div className="flex items-center space-x-2">
                                    ...
                                    <span className="mb-2 text-xs text-gray-600 font-bold">{moment(new Date(item.fecha)).fromNow()}</span>
                                </div>
                        ...
                        <div className="mx-auto flex flex-wrap">
                            ...
                            <div className="flex justify-between items-center mt-2">
                                <a href="/courses/{{ $courses_free_detalle->primary_category->title_cleaned }}/1"
                                    className="px-2 py-1 bg-gray-600 text-gray-100 font-bold rounded hover:bg-gray-500">{item.categoria}</a>
                                ...
                                <a href="/{{ $courses_free_detalle->primary_category->title_cleaned }}/{{ $courses_free_detalle->primary_subcategory->title_cleaned }}/1"
                                    className="px-2 py-1 bg-gray-600 text-gray-100 font-bold rounded hover:bg-gray-500">{item.subcategoria}</a>
                            </div>
                            ...
                            <div className="mt-2">
                                <a href="/cursos-gratis" className="text-4xl text-gray-700 font-bold hover:underline">{item.titulo}</a>
                                    ...
                                    <div className="container py-2 mx-auto">
                                        ...
                                        <div className="flex flex-wrap -m-4">
                                            <div className="p-4 w-full">
                                                <nav
                                                    className="flex flex-col sm:items-start sm:text-left text-left items-left -mb-1 space-y-2.5">
                                                    ...
                                                    <a href="" className="my-2">
                                                        ...
                                                        Conceptos Fundamentales Sobre Software Seguro
                                                        La triada: Confidencialidad, Integridad y Disponibilidad
                                                        Clasificación de datos por su estado en el tiempo
                                                    </a>
                                            ...
                                            <div className="p-4 w-full">
                                                <nav className="flex flex-col sm:items-start sm:text-left text-left items-left -mb-1 space-y-2.5">
                                                    <a className="my-2">
                                                        ...
                                                        </span>Fundamentos de programación básicos
                                                    </a>
                                                </nav>
                                            </div>
                                        </div>
                                ...
                                <div className="mt-2 text-gray-600" dangerouslySetInnerHTML={{ __html: item.descripcion }}>
                                </div>
                            ...
                            <div className="my-2">
                                <div className="grid gap-6 md:grid-cols-2">
                                    <div className="flex content-center">
                                        <a href={ `https://click.linksynergy.com/deeplink?id=sIt9MIeGnaM&mid=47900&murl=https%3A%2F%2Fwww.udemy.com%2Fcourse%2F${item.ruta}/?couponCode=${item.cupon}` } rel="nofollow"
                                            target="_blank"
                            ...
                            <section className="text-gray-600 body-font mb-4">
                                <div className="container px-5 py-auto mx-auto">
                                    ...
                                    {relatedCourses.map((coursesrelated)=>(
                                        <div key={coursesrelated.id} className="p-4 md:w-1/3 sm:mb-0 mb-6">
                                            <div className="rounded-lg h-64 overflow-hidden">
                                                <img alt="content" className="object-cover object-center h-full w-full" src={coursesrelated.portada} /> 
                                            </div>
                                            <h2 className="text-xl font-medium title-font text-gray-900 mt-5">{coursesrelated.titulo}</h2>
                                            <p className="text-base leading-relaxed mt-2">{coursesrelated.descripcion_corta}</p>
                                            <Link href={'/coupons-udemy/[ruta]'} as={`/coupons-udemy/${coursesrelated.ruta}`} className="text-indigo-500 inline-flex items-center mt-3">Ir al Curso
                    ...
                    <div className="flex flex-col bg-white max-w-lg px-6 py-4 mx-auto rounded-lg shadow-md">
                        {
                            outstanding.map((items)=>(
                                <Link key={items.id} href={'/coupons-udemy/[ruta]'} as={`/coupons-udemy/${items.ruta}`}>
