...
const idioma = query.language || 'Spanish';
const page = query.page || 1;
const limit = 20;
const { data: courseslanguege } = await axios.get(
    `${url}/language?idioma=${idioma}&page=${page}&limit=${limit}`,{
      headers: {
  ...
  return {
    props: {
      courseslanguege,page,idioma
    }
  }
}
type Props = {
  courseslanguege: Courses[],page,idioma
}
const PageCouponsLanguage = ({ courseslanguege,page,idioma }) => {
  const router = useRouter();
  const routePaginate = `/courses-free/${idioma}?page=`;
  const totalCourses = courseslanguege.coursesByLanguegeCount[0].count;
  const search = null;
    ...
    <Layout>
      ...
      <>
        ...
        <div className="mb-4 w-screen sm:w-full md:full lg:w-full xl:w-full mx-auto grid grid-cols-1 px-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {courseslanguege.coursesByLanguegeCupon.map(coupons => (
...
export default PageCouponsLanguage;